...
class OpcionesController extends AppController{
   ...
   var $pageTitle = 'Modulo Opciones';
   var $uses = array('Opcione', 'Encuesta');
   
   var $model = 'Opcione';
   var $relacion = 'Encuesta';
...
function index($id= null){ 
	...
	$this->{$this->relacion}->recursive = 0; 
	$this->paginate['limit'] = 20;
	$condicion = array('Opcione.encuesta_id' => $id);
	$this->set($this->name, $this->paginate($this->model,$condicion));
  
    $this->set($this->relacion, $this->{$this->relacion}->read(null, $id));
    $this->set('re', $id);
    
  $this->set('msj_errvalidacion','Error');
  $error=0;# para controlar el Opcione de error
	...
	if(!empty($this->data)){
		...
		if ($this->{$this->model}->validates()){
			if ($this->{$this->model}->save($this->data)){	  
				...
				$this->redirect('index/'.$this->data[$this->model]['encuesta_id']);
	        }
		} 
...
function delete($id=null, $re=null)
{
	$this->{$this->model}->del($id);
	$this->Session->setflash('La '.$this->model.' fue eliminada exitosisamente');
    $this->redirect('index/'.$re);
}	 
