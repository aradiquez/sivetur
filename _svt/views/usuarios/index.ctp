...
?>
	...
	<tr <?=($i%2==0)? "class='altRow'": ''?>>
	    ...
	    <?=$html->link('Editar','/usuarios/edit/'.$row['Usuario']['id'])?>
	    <?=$html->link('Eliminar',"/usuarios/delete/{$row['Usuario']['id']}",null,'Esta seguro de eliminar este usuario?')?>
...
<br>
<ul>
    <li><?=$html->link('Nuevo usuario','/usuarios/add')?></li>
</ul>
